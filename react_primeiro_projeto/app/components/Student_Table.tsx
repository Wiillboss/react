...
export const Student_Table = ({ students }: Props) => {
    return (
        <table className="w-full border-collapse border border-gray-700 rounded-b-md ">
            <thead>
                <tr className="text-left border-grai-600 bg-gray-800">
                ...
                {students.map(item => (
                    <tr key={item.id} className="text-black bg-gray-400 border-b border-black">
                        <td className="p-3 flex items-center">
                            <img src={item.avatar} alt={item.name} className="w-10 h10 rounded-full mr-3" />
                            <div>
                        ...
                        <td className="p-3 font-bold">{item.active ? ((item.grade1 + item.grade2) / 2).toFixed(1) : "N/A"}</td>
                    </tr>
                ))}
            </tbody>
        </table>
